...
@streamer_routers.get("/all/", status_code=status.HTTP_200_OK)
...
async def get_streamers_from_twitch(
    url: AnyUrl, offset: Optional[int] = None
) -> list[StreamerOut]:
    list_streamers = get_data_stream(offset=offset, url=url)
...
@streamer_routers.delete("/", status_code=status.HTTP_204_NO_CONTENT)
