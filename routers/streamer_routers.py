...
from kafka_producers.kafka_streamers import (
    consumer_streamer,
...
async def get_streamers_from_twitch(offset: Optional[int] = None, streamer_dao = Depends(StreamerDAO)) -> list[StreamerOut]:
...
async def create_streamer(streamer: StreamerIn, streamer_dao = Depends(StreamerDAO)) -> StreamerIn:
...
async def update_streamer(id: str, streamer_data: StreamerIn, streamer_dao = Depends(StreamerDAO)) -> StreamerIn:
